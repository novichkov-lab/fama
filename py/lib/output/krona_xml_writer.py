...
def make_functions_chart(parser, score='efpkg'):
    ...
    with open(outfile, 'w') as out:
        ...
        out.write(' '.join(['\t<color attribute="identity"', 'valueStart="50"', 'valueEnd="100"',
                            'hueStart="0"', 'hueEnd="240"', 'default="true"></color>\n']))

        # Write dataset
        out.write('\t<datasets>\n')
        out.write('\t\t<dataset>' + parser.sample.sample_id + '</dataset>\n')
        out.write('\t</datasets>\n')
    ...
    with Popen(krona_cmd, stdout=PIPE, bufsize=1, universal_newlines=True) as proc:
    ...
    if tax_profile.tree.data[taxid].children:
        ...
        < tax_profile.tree.data[taxid].attributes['count']:
            if offset == 2:
                ...
                ret_val += '\t'*offset + '<node name="Unidentified '\
