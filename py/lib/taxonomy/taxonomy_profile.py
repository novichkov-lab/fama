...
class TaxonomyProfile(object):
            ...
            if taxid == UNKNOWN_TAXONOMY_ID:
                ...
                for function in scores[taxid]:
                    if 'genes' in scores[taxid][function]:
                        self.tree.add_attribute(taxid, function,
                                                {'genes':scores[taxid][function]['genes']},
                                                taxonomy_data)
                        attributes = {k:v for k, v in scores[taxid][function].items() \
                            if k != 'genes'}
                        self.tree.add_attribute_recursively(taxid, function,
                                                            attributes, taxonomy_data)
                    else:
                        self.tree.add_attribute_recursively(taxid, function,
                                                            scores[taxid][function],
                                                            taxonomy_data)
        ...
        if taxid in self.tree.data:
            ...
            if self.tree.data[taxid].attributes:
                for function in self.tree.data[taxid].attributes:
                    try:
                        ret_val += '\t'*(offset + 5) + function + '\tScore:' + \
                        format((self.tree.data[taxid].attributes[function][score]), "0.3f") \
                        + '\tIdentity:' \
                        + format((self.tree.data[taxid].attributes[function]['identity'] \
                        / self.tree.data[taxid].attributes[function]['hit_count']), "0.1f") \
                        + '%\tRead count:' \
                        + format(self.tree.data[taxid].attributes[function]['count'], "0.0f") + '\n'
                    except TypeError:
            ...
            if self.tree.data[taxid].attributes:
                for function in function_list:
                    if function in self.tree.data[taxid].attributes:
                        line.extend(['\t', format((self.tree.data[taxid].\
                            attributes[function]['rpkm']), "0.3f"), '\t', \
                            format((self.tree.data[taxid].attributes[function]['identity'] \
                            / self.tree.data[taxid].attributes[function]['count']), "0.1f"), \
                            '%\t', \
                            format(self.tree.data[taxid].attributes[function]['count'], "0.0f")])
                    else:
                        line.append('\t'*3)
    ...
    def convert_profile_into_df(self, score='rpkm'):
        ...
        tax_dict, _ = self.convert_node_into_dict(root_id,
                                                  function_list,
                                                  line_number,
                                                  score=score)
            ...
            for child_id in sorted(self.tree.data[taxid].children):
                child_lines, child_attribute_values = \
                    self.convert_node_into_dict(child_id,
                                                ...
                                                line_number,
                                                score)
                for child_line_number, child_line in child_lines:
            ...
            if unidentified_flag and self.tree.data[taxid].rank in LOWER_RANKS:
                ...
                for function in function_list:
                    ...
                    < self.tree.data[taxid].attributes[function]['count']:
                        ...
                        if 'identity' in self.tree.data[taxid].attributes[function] \
                        and self.tree.data[taxid].attributes[function]['hit_count'] \
    ...
    def convert_node_into_values_dict(self, taxid, function_list, line_number, score='efpkg'):
        ...
        if taxid in self.tree.data:
            ...
            if self.tree.data[taxid].children:
                for child_id in sorted(self.tree.data[taxid].children):
                    child_lines, child_values = \
                        self.convert_node_into_values_dict(child_id,
                                                           ...
                                                           line_number,
                                                           score)
                    for child_line_number in child_lines:
                        ret_val[child_line_number] = child_lines[child_line_number]
                    line_number += len(child_lines)
                ...
                for function in function_list:
                    if function in self.tree.data[taxid].attributes \
                    and children_values[function][score] \
                    < self.tree.data[taxid].attributes[function][score]:
                        unidentified_flag = True
                        break
