...
def get_function_scores(project, sample_id = None, metrics=None):
    ...
    for s in project.list_samples():
        ...
        if metrics == 'readcount':
            norm_factor = 1.0
        elif metrics in ['rpkm','fpkm']:
            norm_factor = project.samples[s].rpkm_scaling_factor
        elif metrics in ['rpkg','fpkg']:
        ...
        # Calculate scores
        if metrics in ['readcount','rpkg','rpkm']:
            if project.samples[s].is_paired_end:
                ...
                if 'pe2' in project.samples[s].reads and read_id in project.samples[s].reads['pe2'].keys(): 
                    ...
                    if read_pe2.get_status() == 'function':
                        ...
                        for function in fragment_functions:
                            if function in function_maxbitscores:
                                    ret_val[function][s]['hit_count'] += 1.0 
                                    ret_val[function][s]['identity'] += function_maxbitscores[hit_function]['identity'] 
    ...
    for s in project.list_samples():
        ...
        if metrics in ['readcount','rpkg','rpkm']:
            ...
            for read_id, read in project.samples[s].reads['pe1'].items():
                ...
                for function in read_rpk_scores:
                    if metrics == 'readcount':
                        ret_val[read.taxonomy][function][s][metrics] += 1.0
                    else:
                        ...
                        for hit in hits1:
                            for hit_function in [function for function in hit.get_functions() if function in fragment_functions]:
                                if hit_function in function_maxbitscores and hit.get_bitscore() == function_maxbitscores[hit_function]:
                                    ...
                                    ret_val[fragment_taxonomy][function][s]['identity'] += hit.get_identity()
                                    del function_maxbitscores[hit_function]
...
def generate_sample_report(project, sample_id):
    ...
    if project.samples[sample_id].is_paired_end:
        # Generate FPKM normalized output
        scores_function_taxonomy = get_function_taxonomy_scores(project, sample_id, metrics='fpkm')
        scores_function = get_function_scores(project, sample_id, metrics='fpkm')
        generate_function_sample_xlsx(project, scores_function, metrics='fpkm', sample_id = sample_id)
        generate_function_taxonomy_sample_xlsx(project, scores_function_taxonomy, metrics='fpkm', sample_id = sample_id)
        ...
        outfile = sanitize_file_name(os.path.join(project.options.get_work_dir(), sample_id + '_fpkm_functional_taxonomy_profile.xml'))
        ...
        generate_taxonomy_series_chart(tax_profile, function_list, outfile, score='fpkm')

        # Generate FPKG normalized output
        scores_function = get_function_scores(project, sample_id, metrics='fpkg')
        scores_function_taxonomy = get_function_taxonomy_scores(project, sample_id, metrics='fpkg')
        generate_function_sample_xlsx(project, scores_function, metrics='fpkg', sample_id = sample_id)
        generate_function_taxonomy_sample_xlsx(project, scores_function_taxonomy, metrics='fpkg', sample_id = sample_id)
        ...
        tax_profile = TaxonomyProfile()
        outfile = sanitize_file_name(os.path.join(project.options.get_work_dir(), sample_id + '_fpkg_functional_taxonomy_profile.xml'))
        tax_profile.build_functional_taxonomy_profile(project.taxonomy_data, sample_scores_taxonomy)
        function_list = sorted(project.ref_data.functions_dict.keys())
        generate_taxonomy_series_chart(tax_profile, function_list, outfile, score='fpkg')

    else:
        #rpk_scores_taxonomy = get_function_taxonomy_scores(project, sample_id, metrics='rpkm')
        rpk_scores = get_function_scores(project, sample_id, metrics='rpkm')
        scores_function_taxonomy = get_function_taxonomy_scores(project, sample_id, metrics='rpkm')
        generate_function_sample_xlsx(project, 
                                    rpk_scores_taxonomy, 
                                    metrics='rpkm', 
                                    norm_factor = project.samples[sample_id].rpkm_scaling_factor, 
                                    sample_id = sample_id)
        generate_function_taxonomy_sample_xlsx(project, scores_function_taxonomy, metrics='rpkm', sample_id = sample_id)
        sample_scores_taxonomy = autovivify(3, float)
        for tax in scores_function_taxonomy.keys():
            for f in scores_function_taxonomy[tax].keys():
                if sample_id in scores_function_taxonomy[tax][f]:
        ...
        tax_profile = TaxonomyProfile()
        outfile = sanitize_file_name(os.path.join(project.options.get_work_dir(), sample_id + '_rpkm_functional_taxonomy_profile.xml'))
        tax_profile.build_functional_taxonomy_profile(project.taxonomy_data, sample_scores_taxonomy)
        function_list = sorted(project.ref_data.functions_dict.keys())
        generate_taxonomy_series_chart(tax_profile, function_list, outfile, score='rpkm')


        rpk_scores = get_function_scores(project, sample_id, metrics='rpkg')
        scores_function_taxonomy = get_function_taxonomy_scores(project, sample_id, metrics='rpkg')
        generate_function_sample_xlsx(project, 
                                    rpk_scores_taxonomy, 
                                    metrics='rpkg', 
                                    norm_factor = project.samples[sample_id].rpkg_scaling_factor, 
                                    sample_id = sample_id)
        generate_function_taxonomy_sample_xlsx(project, scores_function_taxonomy, metrics='rpkg', sample_id = sample_id)
        sample_scores_taxonomy = autovivify(3, float)
        for tax in scores_function_taxonomy.keys():
            for f in scores_function_taxonomy[tax].keys():
                if sample_id in scores_function_taxonomy[tax][f]:
        ...
        tax_profile = TaxonomyProfile()
        outfile = sanitize_file_name(os.path.join(project.options.get_work_dir(), sample_id + '_rpkg_functional_taxonomy_profile.xml'))
        tax_profile.build_functional_taxonomy_profile(project.taxonomy_data, sample_scores_taxonomy)
        function_list = sorted(project.ref_data.functions_dict.keys())
        generate_taxonomy_series_chart(tax_profile, function_list, outfile, score='rpkg')
#        generate_sample_fpkm_chart(project, sample_id, rpk_scores_taxonomy, metrics='rpkm')
#        generate_sample_fpkg_chart(project, sample_id, rpk_scores_taxonomy, metrics='rpkg')
...
def generate_project_report(project):
    ...
    print ('Generating spreadsheets and innteractive diagrams for the project...')
    if is_paired_end:
        scores = get_function_scores(project, sample_id=None, metrics='fpkm')
        generate_function_sample_xlsx(project, scores, metrics='fpkm')
        scores_function_taxonomy = get_function_taxonomy_scores(project, sample_id=None, metrics='fpkm')
        generate_function_taxonomy_sample_xlsx(project, scores_function_taxonomy, metrics='fpkm', sample_id=None)
        scores = get_function_scores(project, sample_id=None, metrics='fpkg')
        generate_function_sample_xlsx(project, scores, metrics='fpkg')
        scores_function_taxonomy = get_function_taxonomy_scores(project, sample_id=None, metrics='fpkg')
        generate_function_taxonomy_sample_xlsx(project, scores_function_taxonomy, metrics='fpkg', sample_id=None)
    else:
        scores = get_function_scores(project, sample_id=None, metrics='rpkm')
        generate_function_sample_xlsx(project, scores, metrics='rpkm')
        scores_function_taxonomy = get_function_taxonomy_scores(project, sample_id=None, metrics='rpkm')
        generate_function_taxonomy_sample_xlsx(project, scores_function_taxonomy, metrics='rpkm', sample_id=None)
        scores = get_function_scores(project, sample_id=None, metrics='rpkg')
        generate_function_sample_xlsx(project, scores, metrics='rpkg')
        scores_function_taxonomy = get_function_taxonomy_scores(project, sample_id=None, metrics='rpkg')
        generate_function_taxonomy_sample_xlsx(project, scores_function_taxonomy, metrics='rpkg', sample_id=None)
